...
class AgentService:
    ...
    async def stream_request(self, user_input: str) -> AsyncGenerator[Dict[str, Any], None]:
    ...
    async def process_request(self, user_input: str) -> Dict[str, Any]:
        """Process user input and return complete agent response"""
        if not self._agent:
            await self.initialize_agent()
...
def display_streaming_response(container, chunk: Dict[str, Any], accumulated_content: Dict[str, str]) -> None:
    ...
    for agent_name, agent_data in chunk.items():
        if 'messages' in agent_data:
            for message in agent_data['messages']:
                ...
                if message_type == 'ai' and hasattr(message, 'content') and message.content:
                    if agent_name not in accumulated_content:
                        accumulated_content[agent_name] = ""
                    ...
                    accumulated_content[agent_name] += content

                    with container:
                        st.markdown("**🤖 Assistant Response:**")
                        st.markdown(accumulated_content[agent_name])
...
async def handle_streaming_request(agent_service: AgentService, user_input: str,
                                   stream_container, status_container) -> Dict[str, Any]:
    """Handle streaming request with real-time updates"""
    accumulated_content = {}
    final_response = None

    try:
        status_container.info("🔄 Starting request processing...")

        async for chunk in agent_service.stream_request(user_input):
            display_streaming_response(stream_container, chunk, accumulated_content)
            final_response = chunk  # Keep the latest chunk as final response

        status_container.success("✅ Response completed!")
...
def main():
    ...
    # Page configuration
    st.set_page_config(
        page_title="Professional AI Agent Demo (Streaming)",
        page_icon="🤖",
        layout="wide",
    ...
    # Header
    st.title("🤖 Professional AI Agent Demo (Streaming)")
    st.markdown("---")
    st.markdown("**Powered by Amazon Bedrock & Claude 3.5 Sonnet with Live Streaming**")
    ...
    with st.sidebar:
        ...
        use_streaming = st.checkbox("Enable Live Streaming", value=True)
        ...
        if submit_button and user_input.strip():
            ...
            try:
                if use_streaming:
                    ...
                    status_container.success("✅ Response generated successfully!")
                ...
                status_container.error(f"❌ Agent Error: {e}")
            except Exception as e:
    ...
    if st.session_state.conversation_history:
        ...
        if not latest.get('streaming', False):
            # Only show detailed trace for non-streaming responses
            if 'messages' in latest['response']:
        ...
        if len(st.session_state.conversation_history) > 1:
            ...
            with st.expander("View Previous Conversations", expanded=False):
                for i, conv in enumerate(reversed(st.session_state.conversation_history[:-1])):
                    ...
                    # Try to extract final response content
                    try:
                        if 'messages' in conv['response']:
                                ...
                                if isinstance(content, list):
                                    ...
                                    for block in content:
                                        if hasattr(block, 'text'):
                                            ...
                                            content_str += str(block)
                                    content = content_str
                                st.markdown(f"*Assistant:* {str(content)[:100]}...")
                        else:
                            # Handle streaming response format
                            for agent_name, agent_data in conv['response'].items():
                                if 'messages' in agent_data:
                                    for message in agent_data['messages']:
                                        if hasattr(message, 'content') and message.content:
                                            content = message.content
                        ...
                        st.markdown("*Assistant:* [Response available]*")
    ...
    st.markdown("---")
    st.markdown(
        "*Built with Streamlit, Amazon Bedrock, and Claude 3.5 Sonnet with real-time streaming capabilities. "
        "This demo showcases professional AI agent capabilities with live tool integration updates.*"
    )
